...
public class CuboidRegion extends AbstractRegion implements FlatRegion {
    ...
    public Set<Region> getEdges(boolean[] flags) {
        ...
        if (min.equals(max))
            ...
            edges.add(createLineEdge(min, max, "x"));
        else if (isLine(pos1.x(), pos2.x(), pos1.z(), pos2.z()))
            edges.add(createLineEdge(min, max, "y"));
    ...
    private Region createLineEdge(BlockVector3 min, BlockVector3 max, String axis) {
        return switch (axis) {
            ...
            case "y" -> new CuboidRegion(min.withY(min.y()), max.withY(max.y()));
            case "z" -> new CuboidRegion(min.withZ(min.z()), max.withZ(max.z()));
